...
urlpatterns = [
    ...
    path('client_list.html', cache_page(60)(ClientListView.as_view()), name='client_list'),  # Список клиентов
    path('client_update/<int:pk>/', ClientUpdateView.as_view(), name='client_update'),  # Изменение клиента
    path('client_delete/<int:pk>/', cache_page(60)(ClientDeleteView.as_view()), name='client_delete'),  # Удаление клиента

    path('message_form.html', MessageCreateView.as_view(), name='message_form'),  # Форма для создания письма
    path('message_list.html', cache_page(60)(MessageListView.as_view()), name='message_list'),  # Список писем для рассылки
    path('message_update/<int:pk>/', MessageUpdateView.as_view(), name='message_update'),  # Изменение письма
    path('message_delete/<int:pk>/', cache_page(60)(MessageDeleteView.as_view()), name='message_delete'),  # Удаление клиента

    path('mailings_form.html', MailingCreateView.as_view(), name='mailings_form'),  # Форма для создания рассылки
    path('mailings_list.html', cache_page(60)(MailingListView.as_view()), name='mailings_list'),  # Список рассылок
    ...
    path('mailings_detail/<int:pk>/', cache_page(60)(MailingDetailView.as_view()), name='mailings_detail'),  # Просмотр рассылки
